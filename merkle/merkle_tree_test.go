...
func TestCompactMerkleTree_GetRootWithNewLeaves(t *testing.T) {
	...
	leaves := make([]web3.Hash, N)
	for i := 0; i < N; i++ {
		leaves[i][:][0] = byte(i)
		hash := leaves[i]
		assert.Equal(t, tree1.GetRootWithNewLeaf(hash), tree2.GetRootWithNewLeaves([]web3.Hash{hash}))
