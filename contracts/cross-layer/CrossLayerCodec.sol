...
library CrossLayerCodec {
    ...
    function crossLayerMessageHash(address _target, address _sender, uint64 _messageIndex, bytes memory _message)
        internal
        pure
        returns (bytes32)
    {
    ...
    function encodeL2ToL1CallData(address _target, address _sender, bytes memory _message, uint64 _messageIndex)
        internal
        pure
        returns (bytes memory)
    {
        return abi.encodeWithSignature(
            "relayMessage(address,address,bytes,uint64)", _target, _sender, _message, _messageIndex
        );
    }
}
