...
library MerkleMountainRange {
    ...
    function appendLeafHash(CompactMerkleTree storage tree, bytes32 leaf)internal {
        ...
        hashes.push(leaf);
        tree.rootHash = bytes32(0);
    }

    function verifyLeafHashInclusion(bytes32 _leafHash,uint64 _leafIndex,bytes32[] memory _proof,bytes32 _rootHash,uint64 _treeSize)internal pure {
        require(_leafIndex<_treeSize,"leaf index out of bounds");
        require(calculateRootHashFromAuditPath(_leafHash,_leafIndex,_proof,_treeSize)==_rootHash,"Constructed root hash differs from provided root hash");
    }

    function calculateRootHashFromAuditPath(bytes32 _leafHash,uint64 _leafIndex,bytes32[] memory _auditPath,uint64 _treeSize)internal pure returns(bytes32){
