...
library MerkleTrie {
     ...
     */
    function get(mapping(bytes32 => HashDB.Preimage) storage _hashdb, bytes memory _key, bytes32 _root)
        internal
        view
        returns (bool _exists, bytes memory _value)
    {
        ...
        (TrieNode[] memory proof, uint256 pathLength, bytes memory keyRemainder, bool isFinalNode) =
            _walkNodePath(_hashdb, key, _root);
     ...
     */
    function _walkNodePath(mapping(bytes32 => HashDB.Preimage) storage _hashdb, bytes memory key, bytes32 _root)
        private
        view
        returns (TrieNode[] memory _proof, uint256 _pathLength, bytes memory _keyRemainder, bool _isFinalNode)
    {
        ...
        for (uint256 i = 0; i < _proof.length; i++) {
            ...
            if (currentNodeLength >= 32) {
                ...
                currentNode =
                    getRawNode(BytesSlice.slice(abi.encodePacked(currentNodeID), 0, currentNodeLength).toBytes());
            }
            ...
            if (currentKeyIndex == 0) {
                ...
                require(BytesSlice.toBytes32(currentNode.encoded) == currentNodeID, "Invalid internal node hash");
            }
            ...
            if (currentNode.decoded.length == BRANCH_NODE_LENGTH) {
                if (currentKeyIndex == key.length) {
                    ...
                    Slice memory nextNode = currentNode.decoded[branchKey];
    ...
    function _getNewPath(
        ...
        if (matchLeaf) {
            ...
            if (keyRemainder.length == 0) {
                ...
                // to our branch node.
                newNodes[totalNewNodes] = _makeLeafNode(BytesSlice.slice(keyRemainder, 1).toBytes(), _value);
                totalNewNodes += 1;
            }
        } else {
            ...
            if (sharedNibbleLength != 0) {
                ...
                lastNodeKey = BytesSlice.slice(lastNodeKey, sharedNibbleLength).toBytes();
                keyRemainder = BytesSlice.slice(keyRemainder, sharedNibbleLength).toBytes();
            }
            ...
            if (lastNodeKey.length == 0) {
                ...
                lastNodeKey = BytesSlice.slice(lastNodeKey, 1).toBytes();
            ...
            if (keyRemainder.length == 0) {
                ...
                keyRemainder = BytesSlice.slice(keyRemainder, 1).toBytes();
                // Push the branch into the list of new nodes.
                newNodes[totalNewNodes] = newBranch;
                totalNewNodes += 1;
    ...
    function _getUpdatedTrieRoot(
        ...
        for (uint256 i = _nodes.length; i > 0; i--) {
            ...
            if (currentNodeType == NodeType.LeafNode) {
                ...
                key = BytesSlice.slice(key, 0, key.length - nodeKey.length).toBytes();
                ...
                if (previousNodeHash.length > 0) {
                    ...
                    key = BytesSlice.slice(key, 0, key.length - 1).toBytes();
    ...
    function _parseProof(bytes memory _proof) private pure returns (TrieNode[] memory _parsed) {
        Slice[] memory nodes = RLPReader.readList(_proof);
     ...
     */
    function _getNodeID(Slice memory _node) private pure returns (bytes32 _nodeID, uint256 length) {
        bytes memory nodeID;

        if (_node.len < 32) {
            // Nodes smaller than 32 bytes are RLP encoded.
            nodeID = _node.toBytes();
        } else {
            // Nodes 32 bytes or larger are hashed.
            nodeID = RLPReader.readBytes(_node);
        }

        return (BytesSlice.toBytes32(nodeID), _node.len);
    }
    ...
    function _getNodePath(TrieNode memory _node) private pure returns (bytes memory _path) {
        return BytesSlice.toNibbles(RLPReader.readBytes(_node.decoded[0]));
    }
    ...
    function _getNodeValue(TrieNode memory _node) private pure returns (bytes memory _value) {
        ...
        if (itemType == RLPReader.RLPItemType.DATA_ITEM) {
            return BytesSlice.toBytes(Slice({ptr: _in.ptr + itemOffset, len: itemLength}));
        } else if (itemType == RLPReader.RLPItemType.LIST_ITEM) {
            require(_in.len < 32, "bad _getNodeValue list");
            return BytesSlice.toBytes(Slice({ptr: _in.ptr, len: _in.len}));
        }
     ...
     * @return _node Node as a TrieNode struct.
     */
    function _makeNode(Slice[] memory _items) private pure returns (TrieNode memory _node) {
        bytes[] memory raw = new bytes[](_items.length);
        for (uint256 i = 0; i < _items.length; i++) {
            raw[i] = _items[i].toBytes();
        }
        ...
        returns (TrieNode memory _node)
    {
        ...
        raw[0] = RLPWriter.writeBytes(BytesSlice.fromNibbles(key));
        ...
        returns (TrieNode memory _updatedNode)
    {
        ...
        _branch.decoded[_branch.decoded.length - 1] = BytesSlice.fromBytes(encoded);
        ...
        returns (TrieNode memory _updatedNode)
    {
        ...
        _branch.decoded[_index] = BytesSlice.fromBytes(encoded);
        return _makeNode(_branch.decoded);
    }
    ...
    function _removeHexPrefix(bytes memory _path) private pure returns (bytes memory _unprefixedKey) {
        if (uint8(_path[0]) % 2 == 0) {
            return BytesSlice.slice(_path, 2).toBytes();
        } else {
            return BytesSlice.slice(_path, 1).toBytes();
        }
    }
