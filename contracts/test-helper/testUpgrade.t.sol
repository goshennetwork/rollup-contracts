...
contract testUpgrade is TestBase {
    ...
    // 8.test challenge
    function testUpgradeChallengeFactory() public {
        // deploy ChallengeFactory & related contract & proxy
        ChallengeFactory challengeFactory = new ChallengeFactory();
        Challenge challenge = new Challenge();
        UpgradeableBeacon challengebeacon = new UpgradeableBeacon(address(challenge));
        ...
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            ...
            address(proxyAdmin),
            abi.encodeWithSelector(
                ChallengeFactory.initialize.selector,
                IAddressResolver(addressManager),
                address(challengebeacon),
                10,
                1
            )
        );
        challengeFactory = ChallengeFactory(address(proxy));

        // upgrade ChallengeFactory
        MockChallengeFactory newChallengeFactory = new MockChallengeFactory();

        vm.startPrank(ownerAddress);
        proxyAdmin.upgrade(proxy, address(newChallengeFactory));
        vm.stopPrank();

        newChallengeFactory = MockChallengeFactory(address(proxy));
        // test call newChallengeFactory
        require(newChallengeFactory.return12() == 12, "upgrade fail 12");
    }
}
